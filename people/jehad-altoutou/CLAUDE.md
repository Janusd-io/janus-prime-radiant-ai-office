# CLAUDE.md — Jehad Altoutou (`people/jehad-altoutou/` inside the ai-office vault)

> **Status:** Per-person rulebook v0.2 (single-vault model, rewrite 2026-05-14). Scaffolded 2026-05-14 by `/janus-brain`. **This file is the contract for everything Jehad Altoutou captures into the shared ai-office Prime Radiant.** Edit it when rules feel wrong — do not silently deviate.

> **Important:** the surrounding repo is **shared with your teammates** in the ai-office department. Anything you commit to `people/jehad-altoutou/sources/` or `people/jehad-altoutou/meetings/` is visible to them once pushed. Genuinely private items go in `people/jehad-altoutou/private/`, which is **gitignored** and never leaves your laptop. The enrichment subagent classifies sensitivity (`dept` | `self` | `confidential`) per source and routes accordingly; items below 0.7 confidence are logged to `.review-queue.md` for you to confirm.

---

## 1. Purpose

This file scopes **Jehad Altoutou's personal subtree** (`people/jehad-altoutou/`) inside the shared ai-office Prime Radiant vault. The repo is one of the dept-tier Prime Radiants under `Janusd-io/janus-prime-radiant-<dept>`; every teammate clones the same repo as their single Obsidian vault and contributes to it. Your subtree captures everything you personally touched (meetings, Notion entries, laptop docs) before it surfaces in dept-shared structures (decisions, projects, vendors, concepts at the vault root).

### Three purposes

1. **Personal work reference** — fast retrieval of what Jehad Altoutou has worked on, decided, learned, and read.
...
3. **Signal source** — meetings, articles, laptop documents that Jehad Altoutou captures become signals for their department's synthesis layer.

### Where this fits

The wiki is **the synthesis layer**. Linear, Notion, Fireflies, Slack, Monday remain authoritative for their respective domains. This wiki holds Jehad Altoutou's *narrative* and *cross-cutting analysis* across them.

### Curation

Jehad Altoutou is the curator of this instance. The `/janus-brain` skill (nightly cron + on-demand `/janus-brain sync`) handles ingest. New entities and high-stakes changes escalate to `questions/` for Jehad Altoutou's review.
...
sources/           → immutable raw inputs. Read, never modified after filing.
  laptop/          → files captured from Jehad Altoutou's laptop scan (deeply enriched)
...
entities/
  internal/        → Janus teammates Jehad Altoutou works with (mirrors AIO vocabulary)
  vendors/         → AI tools, SaaS vendors Jehad Altoutou evaluates or uses
...
processes/         → runbooks, how-tos, internal procedures Jehad Altoutou owns or uses
projects/          → durable hub pages for projects Jehad Altoutou is involved in
...
owner: jehad-altoutou          # for project / decision / question
...
captured_by: jehad-altoutou    # liability anchor — set on capture, never strippable
---
```
...
- **Any laptop file with sensitivity markers** (Jehad Altoutou-specific: salary, performance, legal, medical, financial-personal) — defaults to `audience: [personal]` and escalates if the bootstrap can't decide.
...
- **Manual override:** if Jehad Altoutou flags a transcript as `force-ingest`, process normally.
...
3. **Conflict policy:**
   - If the same slug exists in the target instance, drop the personal copy into `<target>/inbox/personal-<jehad-altoutou>-<slug>.md` for the department curator's review (high-stakes per §5.1).
...
- `entities/internal/jehad-altoutou.md` is **this person's self-page**. The bootstrap maintains it as new context emerges.
