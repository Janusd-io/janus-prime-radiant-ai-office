...
owner: michael-bruck
confidence: high
sources: [pr-backup-2026-05-11-decision-skills-stay-skills]
related: [claude-skills, claude]
audience: [department]
...
- Skills are used by Michael and Jehad only. Audience is two people, not an organisation. Plugin packaging is overhead only justified for broader distribution.
- Skill iteration is fast (file edits in a folder); plugin iteration adds a packaging + install cycle.
- Cross-skill orchestration (the `/standup` ↔ `/ai-registry` ↔ `/ai-tool-evaluation` chaining contract) is already proven within the skill substrate.
...
- Skills grow large enough that plugin-level encapsulation provides material clarity.

## Related

- [[claude-skills]] — concept page covering the Claude skill pattern.
