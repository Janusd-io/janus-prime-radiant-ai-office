...
- CRM lock-in is time-critical: Bonaventure is away soon, and the CRM choice has to be made with him in the room. The window is now or post-return, and now is operationally easier.
...
- Password / user-ID issuance workflow via Euclid + Andre is a downstream task in the same template (longer-horizon).
- CRM direction will be reflected in Linear AIR once locked — likely a Stage 1 or Stage 2 [[ai-tool-evaluation]] outcome.
