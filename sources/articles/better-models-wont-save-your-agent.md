...
created: 2026-05-06
description: "Most agent failures are data failures, not model failures. Pinecone Nexus is a Knowledge Engine that compiles enterprise data into structured artifacts agents can query in one step."
tags:
  - "clippings"
---
...
1. **Agentic RAG:** Chunk the 10-K corpus, embed the chunks, and use hybrid retrieval. Let the agent loop: run the query*,* rerank, read the top chunks, and loop until it’s satisfied with the answer.
2. **Coding Agent in a sandbox:** Give the agent access to file-list, page-read, grep, and full-doc-read tools and let it loop. It opens each 10-K, navigates to the capital returns section, parses the table and extracts the answer.
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2Fcbdd74ceefdb9aa83e39320525232329e44fcefc-1104x1528.png&w=3840&q=75)
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2F1ae351fda4a14a871ef9506a31514e80d711eb5d-1440x1252.png&w=3840&q=75)
...
1. An eval set you define per domain (representative tasks with known right answers) with corresponding data sources
2. A library of pre-vetted skills (e.g. document processing, entity extraction, chunking) the agent can compose into solutions.
3. A feedback loop that scores each iteration against the eval signal.
...
We created KRAFTBench (**K** nowledge **R** etrieval **A** ssessment **F** ramework for **T** ext) to address this gap. This harness measures the accuracy, latency, and token cost of responses generated from a consistent composer model ($\text{claude-sonnet-4-6}$) across different retrieval mechanisms. This way any difference in quality, latency, or token cost of the agent task is attributable to retrieval. The three retrieval mechanisms tested were:

- **Coding agent:** Provide $\text{claude-sonnet-4-6}$ with a small read-only file system toolkit ($\verb|list_files|$ , $\verb|read_file|$ , $\verb|find_filecontent|$ , $\verb|find_filename|$ ). No index provided.
...
Each question is given a constraint of 120 seconds and 1M tokens and each agent iterates until it can answer conclusively or exceeds the constraint. The final answer is then graded for accuracy by an LLM judge ($\text{claude-sonnet-4-6}$) against the eval-set's ground truth output.
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2F08610fc36d20f3249824e5f77a4ee35382f992b5-1760x1028.png&w=3840&q=75)
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2F97966f0efd25e7959fd964237ec9a8fa6b1b9504-1440x2036.png&w=3840&q=75)
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2F6bb9fe8fbe84cbb3ff883e2c1ecd1e5c10dc3d7b-1440x2270.png&w=3840&q=75)
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2Fbbbeaf588ae201d760362e00f289a594d493531d-1440x2142.png&w=3840&q=75)

In the coming weeks, we’ll publish a deep dive on KRAFTBench to provide a closer look at this methodology and results. If you’re interested in collaborating on this benchmark, please [reach out to us](https://www.pinecone.io/contact/).
...
Let’s take the example of a legal review agent looking over a set of [CUAD dataset](https://huggingface.co/datasets/theatticusproject/cuad) contracts stored in Box that wants to ask the following question:
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2Fcbeac85c5848fc5bcb039dbacdd86d1067998411-2048x966.jpg&w=3840&q=75)

**Unstructured (parsing):** Unstructured connects through Box's APIs and captures each file’s content and file metadata (including ACLs). It extracts key document elements, tables, and entities from the legal contracts ($\verb|parties|$, $\verb|agreement_date|$) and passes file permission metadata ($\verb|permission_data|$) to ensure users can only see the contracts they have access to.

![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2F120bd2eb03aa71ee5149fac4fefd6f779a7f196b-1440x1966.png&w=3840&q=75)
...
![](https://www.pinecone.io/_next/image/?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Fvr8gru94%2Fproduction%2Ff4958ca48548a77340e6e86e5c31c63efb2bf0db-1440x1472.png&w=3840&q=75)
...
Nexus is available in early access today for a limited number of design partners. If you're interested in working with us to co-design the knowledge engine powering the next wave of agents, come apply for the [Nexus Early Access](https://pinecone.io/lp/nexus-ea/).
